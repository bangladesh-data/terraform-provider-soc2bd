...
func TestAccDatasourceSoc2bdConnectors_basic(t *testing.T) {
	t.Run("Test Soc2bd Datasource : Acc Connectors Basic", func(t *testing.T) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testDatasourceSoc2bdConnectors(networkName1, connectorName, networkName2, connectorName, connectorName),
...
func testDatasourceSoc2bdConnectors(networkName1, connectorName1, networkName2, connectorName2, prefix string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test_dcs1" {
		name = "%s"
	}
	resource "soc2bd_connector" "test_dcs1" {
		remote_network_id = soc2bd_remote_network.test_dcs1.id
		name = "%s"
	}
	resource "soc2bd_remote_network" "test_dcs2" {
		name = "%s"
	}
	resource "soc2bd_connector" "test_dcs2" {
		remote_network_id = soc2bd_remote_network.test_dcs2.id
		name = "%s"
	}
	data "soc2bd_connectors" "all" {
		depends_on = [soc2bd_connector.test_dcs1, soc2bd_connector.test_dcs2]
	}

	output "my_connectors" {
	  	value = [for c in [for conn in data.soc2bd_connectors.all : conn if can(conn.*.name)][0] : c if length(regexall("%s.*", c.name)) > 0]
	}
		`, networkName1, connectorName1, networkName2, connectorName2, prefix)
}

func TestAccDatasourceSoc2bdConnectors_emptyResult(t *testing.T) {
	t.Run("Test Soc2bd Datasource : Acc Connectors - empty result", func(t *testing.T) {
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testSoc2bdConnectorsDoesNotExists(prefix),
...
func testSoc2bdConnectorsDoesNotExists(prefix string) string {
	return fmt.Sprintf(`
		data "soc2bd_connectors" "all_dcs2" {}
		output "my_connectors_dcs2" {
			value = [for c in [for conn in data.soc2bd_connectors.all_dcs2 : conn if can(conn.*.name)][0] : c if length(regexall("%s.*", c.name)) > 0]
		}
	`, prefix)
}
