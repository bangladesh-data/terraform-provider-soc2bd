...
func TestAccSoc2bdResourceCreate(t *testing.T) {
...
func TestAccSoc2bdResourceCreateWithProtocolsAndGroups(t *testing.T) {
	const theResource = "soc2bd_resource.test2"
	remoteNetworkName := test.RandomName()
...
func createResourceWithProtocolsAndGroups(networkName, groupName1, groupName2, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test2" {
	  name = "%s"
	}

    resource "soc2bd_group" "g21" {
      name = "%s"
    }

    resource "soc2bd_group" "g22" {
      name = "%s"
    }

	resource "soc2bd_resource" "test2" {
	  name = "%s"
	  address = "new-acc-test.com"
	  remote_network_id = soc2bd_remote_network.test2.id
      ...
      access {
		group_ids = [soc2bd_group.g21.id, soc2bd_group.g22.id]
      }
	}
	`, networkName, groupName1, groupName2, resourceName, model.PolicyRestricted, model.PolicyAllowAll)
}

func TestAccSoc2bdResourceFullCreationFlow(t *testing.T) {
	const theResource = "soc2bd_resource.test3"
	remoteNetworkName := test.RandomName()
		...
		Steps: []sdk.TestStep{
			{
				...
				Check: acctests.ComposeTestCheckFunc(
					sdk.TestCheckResourceAttr("soc2bd_remote_network.test3", attr.Name, remoteNetworkName),
					sdk.TestCheckResourceAttr(theResource, attr.Name, resourceName),
					sdk.TestMatchResourceAttr("soc2bd_connector_tokens.test31", attr.AccessToken, regexp.MustCompile(".+")),
				),
			},
		},
	})
...
func resourceFullCreationFlow(networkName, groupName, resourceName string) string {
	return fmt.Sprintf(`
    resource "soc2bd_remote_network" "test3" {
      name = "%s"
    }
	
    resource "soc2bd_connector" "test31" {
      remote_network_id = soc2bd_remote_network.test3.id
    }

    resource "soc2bd_connector_tokens" "test31" {
      connector_id = soc2bd_connector.test31.id
    }

    resource "soc2bd_connector" "test32" {
      remote_network_id = soc2bd_remote_network.test3.id
    }
	
    resource "soc2bd_connector_tokens" "test32" {
      connector_id = soc2bd_connector.test32.id
    }

    resource "soc2bd_group" "test3" {
      name = "%s"
    }

    resource "soc2bd_resource" "test3" {
      name = "%s"
      address = "acc-test.com"
      remote_network_id = soc2bd_remote_network.test3.id
      ...
      access {
        group_ids = [soc2bd_group.test3.id]
      }
    }
    `, networkName, groupName, resourceName, model.PolicyRestricted, model.PolicyAllowAll)
}

func TestAccSoc2bdResourceWithInvalidGroupId(t *testing.T) {
	resourceName := test.RandomResourceName()
...
func createResourceWithInvalidGroupId(networkName, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test4" {
	  name = "%s"
	}

	resource "soc2bd_resource" "test4" {
	  name = "%s"
	  address = "acc-test.com"
	  access {
	    group_ids = ["foo", "bar"]
	  }
	  remote_network_id = soc2bd_remote_network.test4.id
	}
	`, networkName, resourceName)
}

func TestAccSoc2bdResourceWithTcpDenyAllPolicy(t *testing.T) {
	const theResource = "soc2bd_resource.test5"
	resourceName := test.RandomResourceName()
	networkName := test.RandomResourceName()
...
func createResourceWithTcpDenyAllPolicy(networkName, groupName, resourceName string) string {
	return fmt.Sprintf(`
    resource "soc2bd_remote_network" "test5" {
      name = "%s"
    }

    resource "soc2bd_group" "g5" {
      name = "%s"
    }

    resource "soc2bd_resource" "test5" {
      name = "%s"
      address = "new-acc-test.com"
      remote_network_id = soc2bd_remote_network.test5.id
      access {
        group_ids = [soc2bd_group.g5.id]
      }
      protocols {
        allow_icmp = true
        tcp {
...
func TestAccSoc2bdResourceWithUdpDenyAllPolicy(t *testing.T) {
	const theResource = "soc2bd_resource.test6"
	remoteNetworkName := test.RandomName()
	groupName := test.RandomGroupName()
	resourceName := test.RandomResourceName()
...
func createResourceWithUdpDenyAllPolicy(networkName, groupName, resourceName string) string {
	return fmt.Sprintf(`
    resource "soc2bd_remote_network" "test6" {
      name = "%s"
    }

    resource "soc2bd_group" "g6" {
      name = "%s"
    }

    resource "soc2bd_resource" "test6" {
      name = "%s"
      address = "acc-test.com"
      remote_network_id = soc2bd_remote_network.test6.id
      access {
        group_ids = [soc2bd_group.g6.id]
      }
      protocols {
        allow_icmp = true
        tcp {
...
func TestAccSoc2bdResourceWithRestrictedPolicyAndEmptyPortsList(t *testing.T) {
	const theResource = "soc2bd_resource.test7"
	remoteNetworkName := test.RandomName()
	groupName := test.RandomGroupName()
	resourceName := test.RandomResourceName()
...
func createResourceWithRestrictedPolicyAndEmptyPortsList(networkName, groupName, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test7" {
	  name = "%s"
	}

    resource "soc2bd_group" "test7" {
      name = "%s"
    }

	resource "soc2bd_resource" "test7" {
	  name = "%s"
	  address = "new-acc-test.com"
	  remote_network_id = soc2bd_remote_network.test7.id
	  access {
	    group_ids = [soc2bd_group.test7.id]
	  }
	  protocols {
	    allow_icmp = true
	    tcp {
...
func TestAccSoc2bdResourceWithInvalidPortRange(t *testing.T) {
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResourceWithRestrictedPolicyAndPortRange(networkName, resourceName, portRange string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test8" {
	  name = "%s"
	}

	resource "soc2bd_resource" "test8" {
	  name = "%s"
	  address = "new-acc-test.com"
	  remote_network_id = soc2bd_remote_network.test8.id
	  protocols {
	    allow_icmp = true
	    tcp {
	      policy = "%s"
...
func TestAccSoc2bdResourcePortReorderingCreatesNoChanges(t *testing.T) {
	const theResource = "soc2bd_resource.test9"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResourceWithPortRange(networkName, resourceName, portRange string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test9" {
	  name = "%s"
	}

	resource "soc2bd_resource" "test9" {
	  name = "%s"
	  address = "acc-test.com"
	  remote_network_id = soc2bd_remote_network.test9.id
	  protocols {
	    allow_icmp = true
	    tcp {
	      policy = "%s"
...
func TestAccSoc2bdResourceSetActiveStateOnUpdate(t *testing.T) {
		...
		Steps: []sdk.TestStep{
			{
				...
				Check: acctests.ComposeTestCheckFunc(
					acctests.CheckSoc2bdResourceExists(theResource),
					acctests.DeactivateSoc2bdResource(theResource),
					acctests.WaitTestFunc(),
					acctests.CheckSoc2bdResourceActiveState(theResource, false),
				),
			},
			{
				Config: createResourceOnlyWithNetwork(terraformResourceName, remoteNetworkName, resourceName),
				Check: acctests.ComposeTestCheckFunc(
					acctests.CheckSoc2bdResourceActiveState(theResource, true),
				),
			},
		},
	})
...
func TestAccSoc2bdResourceReCreateAfterDeletion(t *testing.T) {
	const terraformResourceName = "test10"
	theResource := acctests.TerraformResource(terraformResourceName)
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
		...
		Steps: []sdk.TestStep{
			{
				...
				Check: acctests.ComposeTestCheckFunc(
					acctests.CheckSoc2bdResourceExists(theResource),
					acctests.DeleteSoc2bdResource(theResource, resource.Soc2bdResource),
				),
...
func TestAccSoc2bdResourceImport(t *testing.T) {
	const theResource = "soc2bd_resource.test12"
	remoteNetworkName := test.RandomName()
	groupName := test.RandomGroupName()
	groupName2 := test.RandomGroupName()
	resourceName := test.RandomResourceName()
...
func createResource12(networkName, groupName1, groupName2, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test12" {
	  name = "%s"
	}

    resource "soc2bd_group" "g121" {
      name = "%s"
    }

    resource "soc2bd_group" "g122" {
      name = "%s"
    }

	resource "soc2bd_resource" "test12" {
	  name = "%s"
	  address = "acc-test.com.12"
	  remote_network_id = soc2bd_remote_network.test12.id
	  access {
	    group_ids = [soc2bd_group.g121.id, soc2bd_group.g122.id]
      }
      protocols {
		allow_icmp = true
        tcp  {
...
func genNewGroups(resourcePrefix string, count int) ([]string, []string) {
	...
	for i := 0; i < count; i++ {
		...
		groupsID = append(groupsID, fmt.Sprintf("soc2bd_group.%s.id", resourceName))
	}
...
func TestAccSoc2bdResourceAddAccessServiceAccounts(t *testing.T) {
	const theResource = "soc2bd_resource.test15"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource15(networkName, resourceName string, terraformServiceAccount string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test15" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test15" {
	  name = "%s"
	  address = "acc-test.com.15"
	  remote_network_id = soc2bd_remote_network.test15.id
...
func TestAccSoc2bdResourceAddAccessGroupsAndServiceAccounts(t *testing.T) {
	const theResource = "soc2bd_resource.test16"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource16(networkName, resourceName string, groups, groupsID []string, terraformServiceAccount string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test16" {
	  name = "%s"
	}
	...
	resource "soc2bd_resource" "test16" {
	  name = "%s"
	  address = "acc-test.com.16"
	  remote_network_id = soc2bd_remote_network.test16.id
...
func TestAccSoc2bdResourceAccessServiceAccountsNotAuthoritative(t *testing.T) {
	t.Parallel()
	const theResource = "soc2bd_resource.test17"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource17(networkName, resourceName string, serviceAccounts, serviceAccountIDs []string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test17" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test17" {
	  name = "%s"
	  address = "acc-test.com.17"
	  remote_network_id = soc2bd_remote_network.test17.id
...
func TestAccSoc2bdResourceAccessServiceAccountsAuthoritative(t *testing.T) {
	t.Parallel()
	const theResource = "soc2bd_resource.test13"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource13(networkName, resourceName string, serviceAccounts, serviceAccountIDs []string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test13" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test13" {
	  name = "%s"
	  address = "acc-test.com.13"
	  remote_network_id = soc2bd_remote_network.test13.id
...
func TestAccSoc2bdResourceAccessWithEmptyGroups(t *testing.T) {
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource18(networkName, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test18" {
	  name = "%s"
	}

	resource "soc2bd_resource" "test18" {
	  name = "%s"
	  address = "acc-test.com.18"
	  remote_network_id = soc2bd_remote_network.test18.id
...
func TestAccSoc2bdResourceAccessWithEmptyServiceAccounts(t *testing.T) {
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource19(networkName, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test19" {
	  name = "%s"
	}

	resource "soc2bd_resource" "test19" {
	  name = "%s"
	  address = "acc-test.com.19"
	  remote_network_id = soc2bd_remote_network.test19.id
...
func TestAccSoc2bdResourceAccessWithEmptyBlock(t *testing.T) {
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource20(networkName, resourceName string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test20" {
	  name = "%s"
	}

	resource "soc2bd_resource" "test20" {
	  name = "%s"
	  address = "acc-test.com.20"
	  remote_network_id = soc2bd_remote_network.test20.id
...
func TestAccSoc2bdResourceAccessGroupsNotAuthoritative(t *testing.T) {
	t.Parallel()
	const theResource = "soc2bd_resource.test22"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource22(networkName, resourceName string, groups, groupsID []string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test22" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test22" {
	  name = "%s"
	  address = "acc-test.com.22"
	  remote_network_id = soc2bd_remote_network.test22.id
...
func TestAccSoc2bdResourceAccessGroupsAuthoritative(t *testing.T) {
	t.Parallel()
	const theResource = "soc2bd_resource.test23"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource23(networkName, resourceName string, groups, groupsID []string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test23" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test23" {
	  name = "%s"
	  address = "acc-test.com.23"
	  remote_network_id = soc2bd_remote_network.test23.id
...
func TestGetResourceNameFromID(t *testing.T) {
	cases := []struct {
		...
		{
			input:    "soc2bd_resource.test.id",
			expected: "soc2bd_resource.test",
		},
		{
			input:    "soc2bd_resource.test",
			expected: "",
		},
		{
			input:    "",
...
func TestAccSoc2bdCreateResourceWithFlagIsVisible(t *testing.T) {
...
func TestAccSoc2bdCreateResourceWithFlagIsBrowserShortcutEnabled(t *testing.T) {
...
func TestAccSoc2bdResourceGroupsAuthoritativeByDefault(t *testing.T) {
	t.Parallel()
	const theResource = "soc2bd_resource.test26"
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource26(networkName, resourceName string, groups, groupsID []string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test26" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test26" {
	  name = "%s"
	  address = "acc-test.com.26"
	  remote_network_id = soc2bd_remote_network.test26.id
...
func TestAccSoc2bdResourceDoesNotSupportOldGroups(t *testing.T) {
	remoteNetworkName := test.RandomName()
	resourceName := test.RandomResourceName()
...
func createResource28(networkName, resourceName string, groups, groupsID []string) string {
	return fmt.Sprintf(`
	resource "soc2bd_remote_network" "test28" {
	  name = "%s"
	}

	%s

	resource "soc2bd_resource" "test28" {
	  name = "%s"
	  address = "acc-test.com.28"
	  remote_network_id = soc2bd_remote_network.test28.id
...
func TestAccSoc2bdResourceCreateWithAlias(t *testing.T) {
...
func TestAccSoc2bdResourceGroupsCursor(t *testing.T) {
